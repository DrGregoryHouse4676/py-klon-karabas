...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        actor_objs = [Actor.objects.get_or_create(first_name=f, last_name=l)[0] for f, l in actors]
        ...
        hall, _ = TheatreHall.objects.get_or_create(name="Велика сцена", rows=10, seats_in_row=12)
        base = make_aware(datetime.now() + timedelta(days=1))
        for i in range(3):
            Performance.objects.get_or_create(play=play, theatre_hall=hall, show_time=base + timedelta(days=i, hours=i))
